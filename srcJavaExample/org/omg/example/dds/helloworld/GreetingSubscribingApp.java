...
public class GreetingSubscribingApp {
    public static void main(String[] args) {
        ...
        Topic<def.Greeting> tp = dp.createTopic("My Topic", def.Greeting.class);
        // OR explicitly create TypeSupport, registered with default name:
        // Topic<def.Greeting> tp = dp.createTopic(
        // "My Topic",
        // ctx.createTypeSupport(def.Greeting.class));
        // OR explicitly create TypeSupport, registered with custom name:
        // Topic<def.Greeting> tp = dp.createTopic(
        // "My Topic",
        // ctx.createTypeSupport(def.Greeting.class, "MyType"));

        Subscriber sub = dp.createSubscriber();
        DataReaderListener<def.Greeting> ls = new MyListener();
        DataReader<def.Greeting> dr = sub.createDataReader(
                tp,
    ...
    private static class MyListener extends DataReaderAdapter<def.Greeting> {
        @Override
        public void onDataAvailable(DataAvailableStatus<def.Greeting> status) {
            DataReader<def.Greeting> dr = status.getSource();
            Sample.Iterator<def.Greeting> it = dr.take();
            while (it.hasNext()) {
                Sample<def.Greeting> smp = it.next();
